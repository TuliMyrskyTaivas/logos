...
class Ratios:
    ...
    def icr(self, data: IFRSData) -> Ratio:
        ...
        if operating_profit is None:
            self.logger.warning("not enoung data of ICR: missing operating profit")
            return None

        interest_expense = data.get('interest_expense')
        if interest_expense is None:
            self.logger.warning("not enough data for ICR: missing interest expense")
            return None
    ...
    def leverage(self, data: IFRSData) -> Ratio:
        ...
        if long_term_liabilities is None:
            self.logger.warning("not enough data for leverage: missing long term liabilities")
            return None

        current_liabilities = data.get('current_liabilities')
        if current_liabilities is None:
            self.logger.warning("not enough data for leverage: missing current liabilities")
            return None

        total_equity = data.get('total_equity')
        if total_equity is None:
            self.logger.warning("not enough data for leverage: missing total equity")
            return None
    ...
    def lvgi(self, leverage: Ratio) -> Ratio:
        ...
        if isinstance(leverage, pd.Series):
            ...
            raise TypeError("failed to calculate LVGI: leverage has a wrong type")

    def tata(self, data: IFRSData) -> Ratio:
        """Total Accruals to Total Assets"""
        cfo = data.get('cfo')
        if cfo is None:
            self.logger.warning("not enough data for TATA: missing cfo")
            return None

        net_profit = data.get('net_profit')
        if net_profit is None:
            self.logger.warning("not enough data for TATA: missing net_profit")
            return None

        current_assets = data.get('current_assets')
        if current_assets is None:
            self.logger.warning("not enough data for TATA: missing current assets")
            return None

        non_current_assets = data.get('non_current_assets')
        if non_current_assets is None:
            self.logger.warning("not enough data for TATA: missing non current assets")
            return None
    ...
    def current_ratio(self, data: IFRSData) -> Ratio:
        current_assets = data.get('current_assets')
        if current_assets is None:
            self.logger.warning("not enough data for Current Ratio: missing current assets")
            return None

        current_liabilities = data.get('current_liabilities')
        if current_liabilities is None:
            self.logger.warning("not enough data for Current Ratio: missing current liabilities")
            return None
    ...
    def aqi(self, data: IFRSData) -> Ratio:
        ...
        if fixed_assets is None:
            self.logger.warning("not enough data for AQI: missing fixed assets")
            return None

        current_assets = data.get('current_assets')
        if current_assets is None:
            self.logger.warning("not enough data for AQI: missing current assets")
            return None

        non_current_assets = data.get('non_current_assets')
        if non_current_assets is None:
            self.logger.warning("not enough data for AQI: missing non current assets")
            return None
        ...
        if ratio is None:
            raise TypeError("failed to calculate AQI: ratio has a wrong type")

        asset_quality = 1 - ratio
        if not isinstance(asset_quality, pd.Series):
            raise TypeError("failed to calculate AQI: asset_quality has a wrong type")
    ...
    def gmi(self, data: IFRSData) -> Ratio:
        ...
        if gross_profit is None:
            self.logger.warning("not enough data for GMI: missing gross profit")
            return None

        revenue = data.get('revenue')
        if revenue is None:
            self.logger.warning("not enough data for GMI: missing revenue")
            return None

        gross_margin = self.__safe_divide(gross_profit, revenue)
        if not isinstance(gross_margin, pd.Series):
            raise TypeError("failed to calculate GMI: gross_margin has a wrong type")
    ...
    def sgai(self, data: IFRSData) -> Ratio:
        ...
        sga = data.get('sga')
        if sga is None:
            self.logger.warning("not enough data for SGAI: missing sga")
            return None

        revenue = data.get('revenue')
        if revenue is None:
            self.logger.warning("not enough data for SGAI: missing revenue")
            return None

        sga_ratio = self.__safe_divide(sga, revenue)
        if not isinstance(sga_ratio, pd.Series):
            raise TypeError("failed to calculate SGAI: sga_ratio has a wrong type")
    ...
    def depi(self, data: IFRSData) -> Ratio:
        ...
        if depreciation is None:
            self.logger.warning("not enough data for DEPI: missing depreciation")
            return None

        fixed_assets = data.get('fixed_assets')
        if fixed_assets is None:
            self.logger.warning("not enough data for DEPI: missing fixed assets")
            return None
        ...
        if not isinstance(depreciation_ratio, pd.Series) or not isinstance(ppe_ratio, pd.Series):
            raise TypeError("failed to calculate DEPI: ratios have a wrong type")
    ...
    def dsri(self, data: IFRSData) -> Ratio:
        ...
        if accounts_receivable is None:
            self.logger.warning("not enough data for DSRI: missing accounts receivable")
            return None

        revenue = data.get('revenue')
        if revenue is None:
            self.logger.warning("not enough data for DSRI: missing revenue")
            return None

        receivables_ratio = self.__safe_divide(accounts_receivable, revenue)
        if not isinstance(receivables_ratio, pd.Series):
            raise TypeError("failed to calculate DSRI: receivables_ratio has a wrong type")
    ...
    def sgi(self, data: IFRSData) -> Ratio:
        ...
        revenue = data.get('revenue')
        if revenue is None:
            self.logger.warning("not enough data for SGI: missing revenue")
            return None
    ...
    def rofa(self, data: IFRSData) -> Ratio:
        ...
        net_profit = data.get('net_profit')
        if net_profit is None:
            self.logger.warning("not enough data for ROFA: missing net profit")
            return None

        fixed_assets = data.get('fixed_assets')
        if fixed_assets is None:
            self.logger.warning("not enough data for ROFA: missing fixed assets")
            return None
    ...
    def fat(self, data: IFRSData) -> Ratio:
        ...
        revenue = data.get('revenue')
        if revenue is None:
            self.logger.warning("not enough data for FAT: missing revenue")
            return None

        fixed_assets = data.get('fixed_assets')
        if fixed_assets is None:
            self.logger.warning("not enough data for FAT: missing fixed assets")
            return None
    ...
    def m_score(self, dsri: Ratio, gmi: Ratio, aqi: Ratio, sgi: Ratio,
        ...
        if (dsri is None or gmi is None or aqi is None or sgi is None or
            ...
            self.logger.warning("not enough data for M-Score: some components are missing")
            return None
